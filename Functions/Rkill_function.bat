...
if exist "%Output%\Tools\RKill\rkill.exe" (
	REM Run RKill
	start /WAIT "[RKill] Brainiacs Cleanup Tool v%TOOL_VERSION%" /MAX "%Output%\Tools\RKill\rkill.exe" -l "%Output%\Logs\rkill.log" -w "%Output%\Tools\RKill\rkill_process_whitelist.txt"
	REM Set notes
